...
class QuestionContainerFrame(tk.Frame):
    def __init__(
        ...
        self._question_description_frame = QuestionDescriptionFrame(
            master=self,
            question_type=questioncontainer.question_type,
            question_description=questioncontainer.description,
            foreground=foreground)
        self._question_description_frame.grid(row=0, column=0, sticky='nwse')
        self._examples_frame = QuestionExampleFrame(
            master=self,
            question_type=questioncontainer.question_type,
            question_examples=questioncontainer.examples)
...
class QuestionRequestAndQueenFrame(tk.Frame):
    ...
    def submit_request(self):
        ...
        self._external_func_dict['submit'](question_content, student_name)
        self.refresh(is_scheduled=False)
        self._schedule = self.after(ms=self._interval, func=self.refresh)


class ListView(tk.Frame):
    def __init__(self, master, columns_and_contents, func_send_delete_signal,
                 *args, **kwargs):
        super().__init__(master=master, *args, **kwargs)
        self._master = master
        self.columns, self.contents = columns_and_contents
        self._func_send_delete_signal = func_send_delete_signal
    ...
    def delete_item(self, id, isCanceled):
        self._func_send_delete_signal(id=id, isCanceled=isCanceled)
    ...
    def display_content(self):
        ...
        for i, row in enumerate(self.contents):
            ...
            tk.Button(
                master=self,
                bg="orange",
                command=partial(self.delete_item, id, True)).grid(
                    row=id, column=len(self.columns))
            tk.Button(
                master=self,
                bg="green",
                command=partial(self.delete_item, id, False)).grid(
